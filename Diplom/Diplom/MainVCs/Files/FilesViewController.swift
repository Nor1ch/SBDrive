...
class FilesViewController : UIViewController {
    ...
    private var files: [DiskFileModel] = []
    private var delegate: TableViewForDelegate?
//    private let path = "https://cloud-api.yandex.net/v1/disk/resources?path=disk%3A%2F&preview_crop=false&preview_size=XL&sort=dir%2C%20-"
    ...
    private lazy var labelLostConnection : UILabel = {
       let view = UILabel()
        view.font = Constants.Fonts.headerFont17
        view.text = Constants.Text.Connection.check
        view.textAlignment = .center
        view.lineBreakMode = .byWordWrapping
        view.numberOfLines = 0
        return view
    ...
    private lazy var refreshButton : UIButton = {
        ...
        let title = Constants.Text.Files.refresh
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        refreshButton.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(doReconnect)))
        ...
        viewModel.networkService.getFolderFiles(path: path, offset: 0){ array in
            guard let array = array else {
                ...
                self.view.addSubview(self.labelLostConnection)
                self.labelLostConnection.snp.makeConstraints { make in
    ...
    @objc private func statusObs(notification: Notification){
        if NetworkMonitor.shared.isConnected {
            ...
            DispatchQueue.main.async {
                ...
                self.labelLostConnection.removeFromSuperview()
